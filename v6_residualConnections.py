...
you transform data but then you have a skip connection
with addition from the previous features

here the computation happens from the top to bottom
and basically you have this residual pathway

and you are free to fork off 
from the residual pathway
perform some computation and
then project back to the residual pathway via addition

so you go from the inputs to the targets
only via + and + and + ( hop motion)

the reason this is useful is because during the propagation
remember from micrograd
...
from the loss basically hop through every addition node
all the way to the input
and also fork off into the residual blocks

but basically have this gradient superhighway that 
goes directly from the supervision 
all the way to the input
unimpeded
and then these original( not sure , residual?) blocks 
are usually initialized
in the beginning
so that they contribute very very little if anything
to the residual pathway

they are initialized that way

so in the beginning they are almost kind of like
not there
but then during the optimization they come online over time
and they start to contribute
but at least at the initialization you can go from
directly  supervisioned
to the input
...
'''
from 3.3 Position-wise Feed-Forward Networks
inner layer of feed forward network 
should be multiplied by 4
in terms of channel sizes 

adding a bit of computation and growing that layer
that is in the residual block on the side of 
